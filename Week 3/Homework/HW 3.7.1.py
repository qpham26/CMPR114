...
def main():
    ...
    write('Score_list.txt', scores, lowest, average)
...
def write(name, scores, lowest, average):
    counter = 0
    output = open(name, 'w')
    for num in scores:
        counter +=1
        output.writelines(f'Test score # {counter}: {num}\n')

    output.writelines(f'The lowest score: {lowest}\n')
    output.writelines(f'Average with lowest score dropped: {average}\n')
    output.close()
